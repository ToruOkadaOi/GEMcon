...
def run_algo(script_name: str, env: str, args: list = None):
    cmd = [
        "bash",
        f"src/run_in_{env}.sh",
        f"src/{script_name}",
    ]  # TODO: better way to handle conda envs?? single bash file???
    if args:
        cmd += args
    run_cmd(cmd)
...
if __name__ == "__main__":
    ...
    p.add_argument(
        "--algo",
        default=None,
        help="Algorithm for metabolic modeling (default - gimme for transcriptomic, geckopy for proteomic)",
    )
    args = p.parse_args()
