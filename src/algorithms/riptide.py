...
class RiptideAlgorithm(BaseAlgorithm):
    ...
    def run(self):
        ...
        if hasattr(riptide_result, "pruned") and isinstance(
            riptide_result.pruned, dict
        ):
            ...
            pruned_df = pd.DataFrame(
                {
                    "pruned_reactions": pruned_rxns
                    + [""] * (max_len - len(pruned_rxns)),
                    "pruned_genes": pruned_genes + [""] * (max_len - len(pruned_genes)),
                    "pruned_metabolites": pruned_mets
                    + [""] * (max_len - len(pruned_mets)),
                }
            )
        ...
        if hasattr(riptide_result, "flux_samples") and isinstance(
            ...
            samples_outpath = os.path.join(
                self.output_dir, f"riptide_flux_samples_{base}.tsv"
            )
