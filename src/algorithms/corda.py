...
class CordaAlgorithm(BaseAlgorithm):
    ...
    def run(self):
        ...
        self.model.solver = corda_cfg.get("solver", "glpk")
        ...
        threshold_absent = corda_cfg.get(
            "threshold_absent",
            gene_expr.quantile(corda_cfg.get("quantile_absent", 0.05)),
        )
        threshold_unknown = corda_cfg.get(
            "threshold_unknown",
            gene_expr.quantile(corda_cfg.get("quantile_unknown", 0.25)),
        )
        ...
        threshold_medium = corda_cfg.get(
            "threshold_medium",
            gene_expr.quantile(corda_cfg.get("quantile_medium", 0.75)),
        )
        ...
        ctx_rxns = [
            r.id for r in self.model.reactions if corda_obj.included.get(r.id, False)
        ]
        ...
        conf_path = os.path.join(
            self.output_dir, f"corda_reaction_confidences_{base}.tsv"
        )
