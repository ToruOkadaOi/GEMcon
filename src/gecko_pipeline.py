...
if not model_path:
    model_path = input(
        "\nProvide full path to the expanded/normal metabolic model (.xml): "
    ).strip()
...
df["ENSP"] = df["string_external_id"].str.replace(
    "9606.", "", regex=False
)  # this strips down to just the enseml tag
...
try:
    # Get results
    result = requests.get(f"https://rest.uniprot.org/idmapping/stream/{job_id}").json()
...
merged["protein_gecko_id"] = merged["UniProt"].apply(
    lambda x: f"prot_{x}[c]" if pd.notna(x) else None
)  # to match the print(model.proteins)
...
ec_model_exp = geckopy.experimental.from_copy_number(
    ...
    vol=vol,
    dens=dens,
    water=water,  # are these close to bio.reality? # test values only
...
geckopy.io.write_sbml_ec_model(
    ec_model_exp, f"{mod_dir}/geckopy_context_specific_model_{base}.xml"
)
